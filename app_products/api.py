...
from .serializers import (ProductSerializer, RegisterSerializer,
                          UpdateSerializer, ProductPresentationSerializer,
...
class ListAPI(generics.RetrieveAPIView):
    def get(self, request, *args, **kwargs):
        ...
        type = kwargs["type"]
        if type == 0:
            queryset = ProductPresentation.objects.filter(is_active=True)
            return Response({
                "products":
                ProductPresentationSerializer(queryset, many=True).data
            })
        ...
        if type == 1:
            ...
            return Response({
                "products":
                ProductInventorySerializer(queryset, many=True).data
            })
